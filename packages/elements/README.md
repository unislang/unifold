...
The twenty-one deferred families are explicit, side-effect-free subpath imports so an
...
`name` reflects for native `FormData`; Checkbox and Switch submit `"on"` only while checked, CheckboxGroup
...
`pnpm generate:cem` analyzes all forty-three catalog elements with the official Custom Elements Manifest
