...
`Breadcrumb` is a leaf navigation component with a required accessible `label` and 1 to 32 exact
`{ "id", "label", "href"? }` positions. IDs are unique, every ancestor requires a safe `href`, and
the final position may be linked or plain text. It renders a labelled native `nav` containing an
ordered list, marks the final position `aria-current="page"`, and emits one
`component.activated` intent carrying `{ "itemId", "href" }` for a selected link. `separator` is
the enum-backed `chevron` or `slash`; separators are presentation-only. Authored Schema.org
`BreadcrumbList`/`ListItem` entities remain in the document semantic graph so there is one JSON-LD
publication authority. Load the deferred `@unislang/unifold/breadcrumb` subpath before mounting.
...
`Dialog` accepts 1 to 64 authored children plus required trigger `label` and accessible
`dialogLabel`. It uses the native modal dialog API when present, has a deterministic fallback,
contains fallback focus, makes background branches inert, restores trigger focus, and emits
enum-backed open/close activation intents. Static export uses native `details` disclosure. Load the
deferred `@unislang/unifold/dialog` subpath before mounting.
...
```json
{
  ...
  "dialogLabel": "Account review",
  "$children": [{ "$comp": "Text", "id": "review-copy", "content": "Ready to save." }]
}
```
